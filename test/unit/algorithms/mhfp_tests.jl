...
@testset "MHFP Fingerprint Tests" begin
    ...
    @testset "MHFP Hashing function tests" begin
        ...
        # We can only test the length of the hashed shingling, as the actual numbers in our
        # implementation differ from those gotten from the original authors code. 
        # However, the rng used by numpy seems to differ from the one used in julia, hence, 
        ...
        ## Testing similarity of hashed vectors depending on the similarity of the original
        ## vectors ########################################################################
        
        # The original authors claim that the MinHash can be used to estimate the tanimoto
        # similarity of two sets.
        # However, testing the claim with their own implementation yields that the tanimoto
        # similarity of the hashed vectors indeed relates to the similarity of the sets that
        # were hashed, but it's not a 1:1 relation.
        # Instead, for sets with low similarity, the corresponding hashed vectors have
        # a similarity which is only around half as large.
        # For sets with higher similarity, the similarity of the corresponding hashed 
        # vectors is still smaller than the similarity of the original sets, but the 
        # difference is less pronounced.
        
        # To verify that our hashing implementation behaves similarly, we generate pairs of
        # sets that share a certain amount of entries, respectively, which implies a certain
        # tanimoto similarity of the pair of sets.
        # Then their MinHash vectors are generated, and then the tanimoto similarity is
        # calculated for these vectors.
        # This is repeated 1000 times and the average calculated, to avoid random effects.
        ...
        # Below, the same process is performed with our implementation and the results 
        # compared to those test values.

        # We will generate 2 sets that share 2, 12, 30, 50 and 80 elements, respectively
        ...
        for (i, overlap_radius) in enumerate(overlap_radii)
            ...
            for j in 1:1000  # repeat 1000 times to avoid random factors
                # Random.seed!(j)
                
                # random set of strings of size 100
                test_set = [randstring(25) for k in 1:100]
                
                # split the 100 strings into two sets, sharing 2*overlap_radius elements
                test_set_1 = test_set[begin:50 + overlap_radius]
                test_set_2 = test_set[50 + 1 - overlap_radius:end]

                # As probably almost all non-overlapping strings are pairwise different, we 
                # should get a tanimoto (jaccard) similarity of approximately:
                # 2 / 100 = 0.02,
                # 12 / 100 = 0.12,
                # 30 / 100 = 0.3,
                # 50 / 100 = 0.5 and
                ...
                minhash_1 = mhfp_hash_from_molecular_shingling(test_set_1, mhfp_calc)
                minhash_2 = mhfp_hash_from_molecular_shingling(test_set_2, mhfp_calc)

                # Calculating tanimoto (jaccard) similarity of the hash vectors.
                tanimoto_of_minhash =  length(
                    intersect(minhash_1, minhash_2)) / length(union(minhash_1, minhash_2))
            ...
            # test whether the average tanimoto similarity is off less than 2 % compared to
            # the reference values
            @test avg_minhash_tanimoto_value ≈ ref_minhash_tanimoto_values[i] rtol=0.02
            
        end
        # TODO add test for different parameters for MHFP()
    end
